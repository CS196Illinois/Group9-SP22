...
def retrieveCurrentData(Overall_Stock):
    ...
    #loop through the data and retrieve the last 30 minutes worth of Stock data (ie. remove the data before last 10 minutes)
    for i in range(OS_Numpy.shape[0] - 10):
...
'''Gathers the Final Tensor Array and Compiles the answer to compare to current stock price and returns A String that reports "Increase" and "Decrease"'''
def finalPrediction(Prediction,current):
    value = Prediction[0]
    if (value > current):
        return "Increase"
    if (value < current):
...
some_Data = getArrays('NDAQ','1d','1m','Open')

#temporary created array that stores the stock data as a numpy array, and allows for indexing to get most current price
current_array = some_Data.to_numpy()

#retrieve the last value which indicates the most up to date price in the stock market, used for comparison in final prediction function
current = current_array[current_array.shape[0] - 1]
...
#takes in the current stock price retrieved by Yahoo Finance API Database, and makes the comparison with our model, and returns a string with "Decrease"/"Increase"
print(finalPrediction(y_test_pred,current))